...
func (ts *TaskService) CreateUserSubmission(taskID int, userID int, userFile []byte, fileName string) ServiceError {
	// Define paths
	taskDir := filepath.Join(ts.taskDirectory, fmt.Sprintf("task%d", taskID))
	...
	if _, err := os.Stat(submissionsDir); os.IsNotExist(err) {
		...
		if err != nil {
			return ErrFailedCreateSubmissionDir
		}
	}
	...
	if err != nil {
		return ErrFailedGetSubmissionNumber
	}
